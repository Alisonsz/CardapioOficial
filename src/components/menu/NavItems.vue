...
const itemData = ref({}); // Inicializa como objeto vazio para evitar erros de leitura de propriedades
...
const fetchItems = async () => {
    try {
        const response = await axios.get('https://projetoclick.uploading.net.br/public/api/products?restaurant_id=1');
...
const openAddItemModal = () => {
    itemData.value = {
        ...
        description: "",
        complement: [],
        restaurant_id: 1 // Assuming a fixed restaurant_id
    };
    isEditing.value = false;
    showModalFormItem.value = true;
};
