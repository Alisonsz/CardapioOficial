  ...
  <div>
    <div class="row mb-3 mt-3">
      ...
      <div class="col-4 create-new text-end">
        <button
          @click="showModalFormComplement = true"
          type="button"
    ...
    <div class="pb-4">
      <table class="table mt-3">
        ...
        <tbody>
          <tr v-for="(complement, index) in searchComplements" :key="index">
            ...
            <td class="used-items">
              <span
                v-for="(item, i) in complement.items"
                :key="i"
                class=" ml-1"
                >{{ item.name }}</span
              >
            ...
            <td class="item-actions">
              ...
              <button
                ...
                @click="showModalFormComplement = true, selectItem(index)"
              >
      ...
      <div v-if="allComplement.length === 0">
        <p class="mt-4">
          Você ainda não adicionou nenhum complemento ao seu cardápio. Comece agora para
    ...
    <Teleport to="body">
      <ModalFormComplement :show="showModalFormComplement" @close="showModalFormComplement = false">
        ...
        <template #body>
          <FormComplement
            @close-modal="showModalFormComplement= false"
...
const complementList = ref([]);
const complementTerm = ref('');
const orderBy = ref('');
const allComplement = ref([]);
const complementEditData = ref([]);

const searchComplements = computed(() => {
  if (complementTerm.value === '') {
    return complementList.value;
  }
  return complementList.value.filter(complement => {
    return complement.name.toLowerCase().includes(complementTerm.value.toLowerCase());
  });
});

onMounted(async () => {
  try {
    const response = await axios.get('https://projetoclick.uploading.net.br/public/api/components?restaurant_id=1', {
      headers: {
        Authorization: 'Bearer 3|XjRorVOeQOmIE0tobq8OJDaXGJURPTudEQLoMpxI08290b45'
      }
    });
    allComplement.value = response.data.data;
    complementList.value = allComplement.value;
  } catch (error) {
    console.error('Error fetching components:', error);
  }
});

function saveComplement(complementData) {
  if (typeof complementData === "object") {
    if (typeof complementData.id === "number") {
      allComplement.value[complementData.id] = complementData;
    } else {
      allComplement.value.push(complementData);
    }
  }
  complementEditData.value = [];
}

function selectItem(index) {
  if (typeof index === "number") {
    complementEditData.value = allComplement.value[index];
    complementEditData.value['id'] = index;
  }
}
...
function orderList() {
  switch (orderBy.value) {
    case "name":
      complementList.value = complementList.value.sort((a, b) => a.name.localeCompare(b.name));
      break;
    case "nameasc":
      complementList.value = complementList.value.sort((a, b) => b.name.localeCompare(a.name));
      break;
  }
}
</script>
